...
function planyo_get_prefixed_params (prefix) {
  ...
  while ((results = regex.exec(window.location.href)) != null) {
    if (results[2] && results[2].length < 50)
...
function planyo_is_special_mode(mode) {
  return (mode == "verify_email" || mode == "payment_confirmation" || mode == "cancel" || mode == "cancel_code" || mode == "reservation_details" || mode == "payment_form" || mode == "reservation_list" || mode == "show_cart" || mode == "checkout" || mode == "additional_products_code" || mode == "modify_data_form");
}
...
function planyo_set_element_value(el, value) {
	if (el.is(":checkbox"))
		el.attr ('checked', (value == 'on' || value == 'yes' || value == '1') ? 'checked' : '');
  else
		el.val(value);
}
...
function planyo_form_loaded(code) {
  if (!code || code.substr (0, 5) == "Error") {
	...
	planyo_send_search_form();
      ...
      jQuery('#product_form').submit(planyo_send_modify_data_form);
    }
